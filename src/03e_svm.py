# 03e_svm.py
# Model de clasificare: Support Vector Machine liniar.
#
# Scop:
# 1. Aplicăm SVM pentru clasificarea confidence_class în 3 clase:
#    0 = low confidence, 1 = medium confidence, 2 = high confidence.
# 2. Folosim StandardScaler, deoarece SVM este sensibil la scala variabilelor.
# 3. Alegem parametrul C prin GridSearchCV cu GroupKFold.
# 4. Evaluăm modelul final prin GroupKFold pe uid_participant.
# 5. Salvăm metricile, matricea de confuzie, classification report și coeficienții.
#
# Notă:
# Folosim LinearSVC în loc de SVC(kernel="rbf") deoarece datasetul are peste
# 1.2 milioane de observații. LinearSVC este varianta scalabilă pentru un SVM liniar.
...
print_section("Model 03e: Support Vector Machine liniar")
...
# ------------------------------------------------------------
# 4. GridSearchCV pentru C
# ------------------------------------------------------------
# C mic = regularizare mai puternică.
# C mare = regularizare mai slabă, risc mai mare de overfitting.

print_section("GridSearchCV SVM liniar")

param_grid = {
    "classifier__C": [0.001, 0.01, 0.1, 1, 10, 100],
}

print("Parametri testați:", param_grid)
print("CV: GroupKFold cu 3 folduri")
print("Scoring principal: macro F1")

gkf = GroupKFold(n_splits=N_SPLITS)

base_model = make_svm_model()
...
cv_results_df.to_csv(
    REPORTS_DIR / "03e_svm_grid_search_all_results.csv",
    index=False
)
...
best_params_df.to_csv(
    REPORTS_DIR / "03e_svm_grid_search_best_params.csv",
    index=False
)
...
top_grid.to_csv(
    REPORTS_DIR / "03e_svm_grid_search_top_10.csv",
    index=False
)

print_section("Top combinații SVM")
cols_to_show = [
    "mean_test_f1_macro",
    "std_test_f1_macro",
    "mean_train_f1_macro",
...
print(top_grid[cols_to_show])


# ------------------------------------------------------------
# 5. Evaluare finală cu GroupKFold
# ------------------------------------------------------------

print_section("Evaluare finală SVM cu GroupKFold")
...
print_section("Rezumat SVM")
print(summary_df)


# ------------------------------------------------------------
# 7. Verificare overfitting
# ------------------------------------------------------------
...
print_section("Matrice de confuzie totală SVM")
print(cm_df)
...
print_section("Classification report SVM")
print(report_df)


# ------------------------------------------------------------
# 10. Coeficienți model SVM liniar
# ------------------------------------------------------------
# LinearSVC folosește one-vs-rest pentru multiclasă.
# Fiecare rând corespunde unei clase comparate cu celelalte.

print_section("Coeficienți SVM liniar")

final_model = make_svm_model(C=best_C)
final_model.fit(X, y)
...
coef_df.insert(0, "class", CLASS_LABELS)
coef_df["intercept"] = svm_classifier.intercept_

print(coef_df)

coef_df.to_csv(
    REPORTS_DIR / "03e_svm_coeficienti.csv",
    index=False
)
...
results_df.to_csv(
    REPORTS_DIR / "03e_svm_folduri.csv",
    index=False
)

summary_df.to_csv(
    REPORTS_DIR / "03e_svm_summary.csv",
    index=False
)

cm_df.to_csv(
    REPORTS_DIR / "03e_svm_confusion_matrix.csv"
)

report_df.to_csv(
    REPORTS_DIR / "03e_svm_classification_report.csv"
)

comparison_df.to_csv(
    REPORTS_DIR / "03e_comparatie_modele.csv",
    index=False
)